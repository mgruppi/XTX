...
def init_model(model, args: Dict[str, Union[str, int, float]], vocab_size: int, tokenizer, text_encoder):
...
def packed_rnn(model, x, rnn):
    """ Runs the provided rnn on the input x. Takes care of packing/unpacking.
        ...
        x: list of encoded input dictionaries including token ids and attention (padded).
    ...
    # lengths = torch.tensor([len(n['input_ids'][0]) for n in x],
    #                        dtype=torch.long,
    #                        device=device)
    ...
    # z = [n['input_ids'][0] for n in x]
    # padded_x = util.pad_sequences(z)
