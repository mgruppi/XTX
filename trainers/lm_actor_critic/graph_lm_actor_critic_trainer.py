...
# Custom Imports
from trainers import LMActorCriticTrainer

from agents import LMActorCriticAgent
...
import utils.lm_actor_critic as LMAC
import utils.inv_dyn as InvDyn
import utils.ngram as Ngram
from utils.util import process_action


class GraphLMActorCriticTrainer(LMActorCriticTrainer):
    def __init__(
        self,
        tb: logger.Logger,
        log: Callable[..., None],
        agent: LMActorCriticAgent,
        envs: VecEnv,
        eval_env: JerichoEnv,
    ...
    def train(self):
        ...
        #     self, self.envs)
        obs, infos, states, valid_ids, transitions = LMAC.setup_env(
            self, self.envs
        )
